...
//changed from private to public Jyothi -- revert it back
	public static HashMap<SootMethod, Summary> summaries =
		new HashMap<SootMethod, Summary>();
	
	//--START Jyothi
	/*public static int numSumMtdsP = 0; // passed
	public static int numSumMtdsC = 0; //checked
	public static HashMap<SootMethod, Long> timings = new HashMap<SootMethod, Long>();
	public static HashMap<SootMethod, Integer> usage = new HashMap<SootMethod, Integer>();
	public static HashMap<SootMethod, Integer> calleesMap = new HashMap<SootMethod, Integer>();
	public static HashMap<SootMethod, Integer> fieldEdgesMap = new HashMap<SootMethod, Integer>();*/
	//-- END Jyothi
...
//		new HashSet<String>();
	...
	public static Summary getSummary(SootMethod m) {
		...
		//System.out.println("Summary already newly computing !!"+m);
		//Summary.numSumMtdsC++;
...
//		System.out.println("summary-calc: " + m);
		...
		//commented by Jyothi -- START
		/*if (escapableObjs.size() <= 1) { //Jyothi: what happens when there is only one escaple object.
			summaries.put(m, emptySummary);
			return emptySummary;
		}*/
		//commented by Jyothi -- END
...
//				if (!reachables.contains(src)) continue;
				...
				if (Util.isVisited(ctxSumm, fldSumm, e, ctxHash, true)) {				
					continue;
				}
...
//				if (!reachables.contains(tgt)) continue;
				...
				if (Util.isVisited(ctxSumm, fldSumm, e, ctxHash, false)) {
					continue;
				}
...
//		removeSig(sig);	// for cascaded summary
		...
		timings.put(m, end-start);
		Summary.calleesMap.put(m, callees);
		Summary.fieldEdgesMap.put(m, fieldEdges);
		Summary.usage.put(m, 0);*/
		return s;
	}	
